...
sub amount : Tests(5) {
    ...
    throws_ok { $object->amount(random_integer(0, 49)) } 'Stancer::Exceptions::InvalidAmount', 'Throw an exception if not ok';
    like($@->message, qr/must be an integer and at least 50/sm, 'Exception should have a message');
}
...
sub currency : Tests(12) {
    ...
    throws_ok { $object->currency(random_string(3)) } 'Stancer::Exceptions::InvalidCurrency', 'Throw an exception if not ok';
    like($@->message, qr/Currency must be one of "aud", "cad", "chf", "dkk", "eur", "gbp", "jpy", "nok", "pln", "sek", "usd"/sm, 'Exception should have a message');
}
